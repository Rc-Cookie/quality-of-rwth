...
function onStorageChanged(change) {
  ...
  //     buildHTML();
  // }
  else if (change.videoInfos) tryLoadVideoDownloads();
}
...
async function tryLoadCachedCourses() {
  ...
  allCoursesLoaded =
    (await browser.storage.local.get("courseCacheIsComplete"))
      .courseCacheIsComplete === true;
  courses = cache;
...
async function getHiddenCourses() {
  const storageData = (await browser.storage.sync.get("hiddenCourses"))
    .hiddenCourses;
  if (!storageData) return { ids: [], data: {} };

  const hidden = [];
  for (let idStr of Object.keys(storageData)) hidden.push(parseInt(idStr));
...
async function loadCourses(isRetry) {
  ...
  // Load preferences
  const byTime =
    (await browser.storage.sync.get("courseOrder")).courseOrder !== "name"; // Default to true
  const hidden = (await getHiddenCourses()).ids;
  const maxCourses =
    (await browser.storage.sync.get("maxCourses")).maxCourses || 10;
  ...
  const resp = await fetch(
    "https://moodle.rwth-aachen.de/lib/ajax/service.php?sesskey=" + sesskey,
    {
      method: "post",
      body: JSON.stringify([
        {
  ...
  if (!resp[0].error) {
    ...
    if (allCoursesLoaded && courses.length >= resp[0].data.courses.length) {
      ...
      if (resp[0].data.courses.every((c) => firstOldCourseIds.includes(c.id))) {
        ...
        fetch(
          "https://moodle.rwth-aachen.de/lib/ajax/service.php?sesskey=" +
            sesskey,
          {
            method: "post",
            body: JSON.stringify([
    ...
    !isRetry &&
    (resp[0].exception.errorcode === "servicerequireslogin" ||
      resp[0].exception.errorcode === "invalidsesskey")
  )
    tryFetchSesskey(); // Will re-trigger loadCourses() if successful
  else {
    if (resp[0].exception.errorcode === "servicerequireslogin")
      ...
      bottomMessage.innerText =
        "Your moodle session has timed out, courses could not be refreshed.";
    else {
      bottomMessage.innerText =
        "Failed to refresh courses. Maybe you are not logged in?";
 ...
 */
const linkAction = (url, ctrlKey, shiftKey) => {
  const canOpenEmbededTab = true; //TODO: check for chrome
  browser.storage.sync.get("openInNewTab").then((settings) => {
    const setting = settings.openInNewTab;
    ...
    if ((!setting && ctrlKey) || (setting && !ctrlKey)) {
...
async function buildHTML(message) {
  ...
  if (shownCourses.length === 0) shownCourses = getShownCourses([], maxCourses);
  ...
  if (search && !allCoursesLoaded && shownCourses.length < maxCourses) {
    if (message) bottomMessage.innerText = "Loading older courses | " + message;
  ...
  for (const i in shownCourses) {
    ...
    async function updateCache() {
      if (
        (await browser.storage.sync.get("courseOrder")).courseOrder === "name"
      )
        return;
    ...
    const before = document.createTextNode(
      name.substring(0, name.indexOf(main))
    );
    const after = document.createTextNode(
      name.substring(name.indexOf(main) + main.length)
    );
    ...
    /**
     * Is fired when anything other than the mouse clicks.
     * @param {MouseEvent} e
     */
...
async function tryFetchSesskey() {
  ...
  const html = await fetch(
    "https://moodle.rwth-aachen.de/course/view.php"
  ).then((r) => r.text()); // This page does not exist without courseid, but thus loads quickly
...
async function tryLoadVideoDownloads() {
  ...
  const info = ((await browser.storage.local.get("videoInfos")).videoInfos ||
    {})[tab.url];
  if (!info) return;
  ...
  video1080p.onclick = () =>
    browser.downloads.download({ url: track1080p.url });
...
function searchMatch(item) {
  ...
  for (let i = 0; i < search.length; i++) {
    ...
    if (index < start) {
      ...
      index =
        start + item.slice(start).search("(^|[^IV])" + char + "([^a-zA-Z]|$)");
      if (index < start) return false; // Alias also not found
      ...
      quality++;
    } else if (item.charAt(index) != char)
      // char is lower case but matched upper case
      quality++;
